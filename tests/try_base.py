...
for vt in vsa.VsaType:
    v = vsa.VsaBase(np.zeros(vd), vsa_type=vt)
...
for vt in vsa.VsaType:
    ...
    if vt != vsa.VsaType.HRR:
        ...
        expected_hd = "> {:0.4f} orthogonal vector".format(vsa.hsim(vsa.randvec(vd, vsa_type=vsa.VsaType.HRR), bag_vec))
