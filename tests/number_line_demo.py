...
from vsapy.helpers import serialise_object, deserialise_object
from vsapy.vsa_tokenizer import VsaTokenizer
from vsapy import NumberLine
...
def generate_numberline_graph(num_list):
    ...
    #dfplt = df.loc[:, ['dist', 'Bit Distance', 'HSim_val']]
    dfplt = df.loc[:, ['Distance between points on a 1-D numberline', 'Hamming Similarirty']]
    #dfplt.plot.scatter(x='dist', y='HSim_val', title=f"Hamming Similarity of numberline vectors", subplots=False, marker='o')
    dfplt.plot.scatter(x='Distance between points on a 1-D numberline', y='Hamming Similarirty', title=f"Hamming Similarity of numberline vectors", subplots=False, marker='.')
    plt.show()
...
if __name__ in "__main__":
    ...
    MAX_X_POS = 20000  # We can use a range larger than the vector size provided we specify a 'bits_per_step > 0'
    bits_per_step = 100  # Try a large range and change the quantisation
    plot_step_size = 300
    units = NumberLine(MIN_X_POS, MAX_X_POS, vecdim, vsa_kwargs, bits_per_interval=bits_per_step)
    tens = NumberLine(0, 90, vecdim, vsa_kwargs,  bits_per_interval=0)
    hundreds = NumberLine(0, 900, vecdim, vsa_kwargs,  bits_per_interval=0)

    vec_num_list = []
    for i in range(MIN_X_POS, MAX_X_POS+1, plot_step_size):
        vec_num_list.append((i, units.number_to_vec(i)))
